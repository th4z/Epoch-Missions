...
if (_rangeX==0) exitWith {
	diag_log format["UPSMON: Cannot patrol Sector: %1, Area Marker doesnt exist",_areamarker]; 
};
...
// dont move until an enemy is spotted
...
// dont follow outside of marker area
...
if (_initpos!="ORIGINAL") then {
    ...
    // put the unit on top of a building
        ...
        _currPos = getPos _npc;
        _nowp=true; // dont move if on roof		
    };
};
...
while {_loop} do {
	...
	if ( _isSoldier && !_exit) then {
		...
		if ((_shareinfo=="SHARE")) then {			
			...
			//Requests for radio the enemys position, if it is within the range of acts
...
if (_respawn && {_respawnmax > 0} &&  {!_surrended}) then {
    ...
    // get the areaname and check if its a grid area
